...
class PorosityNormalizer(SystemBasedNormalizer):
    ...
    def normalize_system(self, archive, system, is_representative):
        ...
        if (
            ...
            or system.atoms.labels is None
        ):
            return
        ...
        # parser does not currently parse it correctly.
        original_atoms.set_pbc(True)

        if len(original_atoms) < configuration.min_n_atoms:
            return
        ...
        if len(original_atoms) > configuration.max_n_atoms:
            return

        indices_in_parent = mof_deconstructor.remove_unbound_guest(original_atoms)
        ...
        if porosity_data['PLD'] < configuration.min_pld:
            return

        parent_system = get_topology_original(system.atoms, archive)
        no_guest = porosity_system(original_atoms, porosity_data, indices_in_parent)
        ...
        if no_guest.label == 'MOF':
            try:
                mof_properties(
                    original_atoms, indices_in_parent, parent_system, topology
                )
            except Exception:
                pass
...
def porosity_system(original_atoms, porosity_data, indices_in_parent):
    ...
    if (
        len(number_of_metals) > 0
        and len(number_of_carbon) > 0
        and len(organic_entity_on) > 0
    ):
    ...
    no_guest = System(
        ...
        largest_included_sphere_along_free_sphere_path=porosity_data['lfpd']
        * ureg.angstrom,
...
def mof_properties(original_atoms, indices_in_parent, parent_system, topology):
    atoms = original_atoms[indices_in_parent]
    number_of_metals, _ = mof_deconstructor.metal_coordination_number(atoms)
    if len(number_of_metals) == 0:
        return
    ...
    (
        ...
        porphyrin_checker,
        all_regions,
    ) = mof_deconstructor.secondary_building_units(atoms)
    ...
    (
        ...
        porphyrin_checker,
        all_regions,
    ) = mof_deconstructor.ligands_and_metal_clusters(atoms)
    _, organic_linker, _ = mof_deconstructor.find_unique_building_units(
        list_of_connected_components,
        atom_pairs_at_breaking_point,
        atoms,
        porphyrin_checker,
    ...
    for i, sbu in enumerate(metal_sbus):
        ...
        sbu_indices = [
            map_indices[i] for i in sum(sbu.info['atom_indices_mapping'], [])
        ]
        ...
        mof_group_indices = [
            map_indices[i] for i in sbu.info['atom_indices_mapping'][0]
        ]
        mof_group = System(
    ...
    for i, linker in enumerate(organic_sbus):
        ...
        sbu_indices = [
            map_indices[i] for i in sum(linker.info['atom_indices_mapping'], [])
        ]
        ...
        # Single SBU
        mof_group_indices = [
            map_indices[i] for i in linker.info['atom_indices_mapping'][0]
        ]
        mof_group = System(
            indices=[mof_group_indices],
        ...
        mof_group.sbu_coordination_number = len(linker.info['point_of_extension'])
        if 'sbu_type' in list(linker.info.keys()):
    ...
    for i, ligand in enumerate(organic_linker):
        ...
        mof_group_indices = [
            map_indices[i] for i in ligand.info['atom_indices_mapping'][0]
        ]
        sbu_indices = [
            map_indices[i] for i in sum(ligand.info['atom_indices_mapping'], [])
        ]
...
def zeo_calculation(
    ase_atom, probe_radius=1.8, number_of_steps=5000, high_accuracy=True
):
    """
